"""
backend.py — BloodSetu FastAPI Backend
Full REST API — all routes, no Streamlit dependencies.
"""
...
# ── existing pure-Python modules (no streamlit) ────────────────────────────────
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],          # tightened via allow_origin_regex in production
...
# Serve the built React app in production (optional — Vercel handles this)
...
def login(req: LoginReq):
    # Accept both admin credential sets
    if req.username in ("admin", "bloodsetu_admin") and req.password in ("admin", "BloodSetu@2026"):
        user = {"id": 0, "username": req.username, "role": "admin", "phone": ""}
...
def register(req: RegisterReq):
    ...
    if not ok:
        raise HTTPException(status_code=409, detail="Username already taken — try another")
...
def stats():
    ...
    conn.close()
    return {
        "donors": donors, "hospitals": hospitals, "blood_banks": banks,
        "camps": camps, "donations": donations, "active_sos": active_sos,
...
def post_sos(req: SosReq):
    ...
    c.execute("""INSERT INTO sos_requests
        ...
        VALUES (?,?,?,?,?,?,?,?,1)""",
        (req.blood_group, req.city, req.area, req.seeker_name, req.seeker_phone, req.urgency, now, expires))
    conn.commit()
    conn.close()
    msg = wa_sos_message(req.blood_group, req.area, req.city, req.seeker_phone or "Contact BloodSetu")
...
def search(blood_group: str, city: str, area: str):
    ...
    return tier_search(blood_group, city, area)
...
def get_my_donor_profile(user=Depends(require_role("donor"))):
    ...
    return {**donor, "is_eligible": is_eligible, "days_since": days_since,
            "days_remaining": days_remaining, "progress": progress, "badges": badges}
...
def donor_history(user=Depends(require_role("donor"))):
    donor = get_donor_by_user(user["id"])
    return get_donor_donations(donor["id"]) if donor else []
...
def book_slot(req: SlotBookReq, user=Depends(require_role("donor"))):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO donation_slots (donor_id,location_type,location_id,slot_date,slot_time) VALUES (?,?,?,?,?)",
...
def daata_wall():
    ...
    return [{**d, "badges": get_earned_badges(d.get("donations_count", 0), d.get("blood_group", ""))}
            for d in donors]
...
def create_hospital_profile(req: HospitalProfileReq, user=Depends(require_role("hospital"))):
    ...
    if c.fetchone():
        conn.close(); raise HTTPException(status_code=409, detail="Profile exists")
    ...
    c.execute("""INSERT INTO hospitals (user_id,name,doctor_name,address,city,area,phone,emergency_24x7,last_updated,update_due)
                 VALUES (?,?,?,?,?,?,?,?,?,?)""",
              (user["id"], req.name, req.doctor_name, req.address, req.city, req.area,
               req.phone, 1 if req.emergency_24x7 else 0, today, due))
    conn.commit(); conn.close()
    return {"ok": True}
...
def hospital_slots(user=Depends(require_role("hospital"))):
    ...
    c.execute("""SELECT ds.*, d.name as donor_name, d.blood_group, d.phone as donor_phone
                 FROM donation_slots ds JOIN donors d ON ds.donor_id=d.id
                 WHERE ds.location_type='Hospital' AND ds.location_id=? AND ds.status='Pending'
                 ORDER BY ds.slot_date, ds.slot_time""", (row[0],))
...
def hospital_find_donors(blood_group: str, city: str, area: str, user=Depends(require_role("hospital"))):
    return get_eligible_donors(get_compatible_groups(blood_group), city, area)

@app.post("/api/hospital/fraud-report")
def hospital_report_fraud(req: FraudReportReq, user=Depends(require_role("hospital"))):
    _insert_fraud_report(req.reported_phone, "Hospital", user["id"], req.reason)
    return {"ok": True}
...
def hospital_wa(user=Depends(require_role("hospital"))):
    ...
    if not h:
        return {"message": ""}
    h = dict(h)
    return {"message": wa_event_message(h["name"], h["city"], h["area"],
                                        date.today().isoformat(), "9AM - 4PM",
                                        h.get("doctor_name", ""), h["phone"])}
...
def create_bank_profile(req: BloodBankProfileReq, user=Depends(require_role("blood_bank"))):
    ...
    c.execute("INSERT INTO blood_banks (user_id,name,doctor_name,city,area,phone,last_updated) VALUES (?,?,?,?,?,?,?)",
              (user["id"], req.name, req.doctor_name, req.city, req.area, req.phone, date.today().isoformat()))
    conn.commit(); conn.close()
    return {"ok": True}
...
def update_bank_inventory(req: InventoryReq, user=Depends(require_role("blood_bank"))):
    ...
    c.execute("UPDATE blood_banks SET groups_available=?,last_updated=? WHERE user_id=?",
              (",".join(req.groups), date.today().isoformat(), user["id"]))
    conn.commit(); conn.close()
    return {"ok": True}
...
def bank_slots(user=Depends(require_role("blood_bank"))):
    ...
    c.execute("""SELECT ds.*, d.name as donor_name, d.blood_group, d.phone as donor_phone
                 FROM donation_slots ds JOIN donors d ON ds.donor_id=d.id
                 WHERE ds.location_type='Blood Bank' AND ds.location_id=? AND ds.status='Pending'
                 ORDER BY ds.slot_date, ds.slot_time""", (row[0],))
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows
...
def bank_report_fraud(req: FraudReportReq, user=Depends(require_role("blood_bank"))):
    _insert_fraud_report(req.reported_phone, "Blood Bank", user["id"], req.reason)
    return {"ok": True}

@app.get("/api/blood-bank/wa-message")
def bank_wa(user=Depends(require_role("blood_bank"))):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM blood_banks WHERE user_id=?", (user["id"],))
    ...
    if not b:
        return {"message": ""}
    b = dict(b)
    return {"message": wa_event_message(b["name"], b["city"], b["area"],
                                        date.today().isoformat(), "9AM - 4PM",
                                        b.get("doctor_name", ""), b["phone"])}
...
def camp_slots(user=Depends(require_role("camp"))):
    ...
    c.execute("""SELECT ds.*, d.name as donor_name, d.blood_group, d.phone as donor_phone
                 FROM donation_slots ds JOIN donors d ON ds.donor_id=d.id
                 WHERE ds.location_type='Blood Camp' AND ds.location_id=? AND ds.status='Pending'
                 ORDER BY ds.slot_date""", (row[0],))
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows
...
def camp_report_fraud(req: FraudReportReq, user=Depends(require_role("camp"))):
    _insert_fraud_report(req.reported_phone, "Blood Camp", user["id"], req.reason)
    return {"ok": True}
...
def camp_wa(user=Depends(require_role("camp"))):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM blood_camps WHERE user_id=?", (user["id"],))
    camp = c.fetchone(); conn.close()
    if not camp:
        return {"message": ""}
    c2 = dict(camp)
    return {"message": wa_event_message(c2["organizer"], c2["city"], c2["area"],
...
def admin_fraud_reports(admin=Depends(require_admin)):
    conn = get_conn(); c = conn.cursor()
...
def public_hospitals(city: Optional[str] = None):
    ...
    return [h for h in hospitals if h["city"] == city] if city else hospitals

@app.get("/api/blood-banks")
def public_banks(city: Optional[str] = None):
    banks = get_all_blood_banks()
    return [b for b in banks if b["city"] == city] if city else banks

@app.get("/api/camps")
def public_camps(city: Optional[str] = None):
    camps = get_all_camps()
    return [c for c in camps if c["city"] == city] if city else camps


# ── Internal helpers ───────────────────────────────────────────────────────────
def _insert_fraud_report(phone: str, reported_by_type: str, reported_by_id: int, reason: str):
    conn = get_conn(); c = conn.cursor()
    c.execute("""INSERT INTO fake_reports (reported_phone,reported_by_type,reported_by_id,reason,reported_at)
                 VALUES (?,?,?,?,?)""",
              (phone, reported_by_type, reported_by_id, reason, datetime.now().isoformat()))
    conn.commit(); conn.close()
